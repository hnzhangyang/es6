...
- [总结](#总结)
## Map
对象是键值对的集合，**键** 只能是字符串，**值** 可以是任意值，ES6 Map 扩展了这一方面，Map 的 **键和值** 都可以是任意值。
``` javaScript
var map = new Map()
var foo = ['bar']
...
// value
```
这么多限制，WeakMap 要它有什么用？值得注意的是 WeakMap 只能保存 **引用类型** 的 **键**，所以一般我们利用这一特性用 WeakMap 保存一份目标对象的引用，让 WeakMap 成为目标对象不被浏览器垃圾回收机制回收的最后把关。
